...
def main():
    ...
    parser.add_argument('--secrets_provider_config', required=False,
    ...
    parser.add_argument('--install_usercode_dependencies', required=False,
                        help='For packaged python like wheel files, do we need to install all dependencies', type=bool)
    parser.add_argument('--dependency_repository', required=False,
                        help='For packaged python like wheel files, which repository to pull the dependencies from')
    parser.add_argument('--extra_dependency_repository', required=False,
                        help='For packaged python like wheel files, any extra repository to pull the dependencies from')
    ...
    if os.path.splitext(str(args.py))[1] == '.whl':
        ...
        if os.path.isdir(deps_dir) and os.listdir(deps_dir):
            ...
            wheel_file_list = [os.path.join(deps_dir, f) for f in os.listdir(deps_dir) if
                               os.path.isfile(os.path.join(deps_dir, f)) and os.path.splitext(f)[1] == '.whl']
