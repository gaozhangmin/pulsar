...
@Slf4j
public class WindowFunctionExecutor<X, T> implements Function<X, T> {

    protected WindowConfig windowConfig;
    protected transient WaterMarkEventGenerator<Record<X>> waterMarkEventGenerator;
    protected java.util.function.Function<Collection<X>, T> bareWindowFunction;
    protected WindowFunction<X, T> windowFunction;
    private boolean initialized;
    private WindowManager<Record<X>> windowManager;
    private TimestampExtractor<X> timestampExtractor;
    ...
    private WindowManager<Record<X>> getWindowManager(WindowConfig windowConfig, Context context) {

        WindowLifecycleListener<Event<Record<X>>> lifecycleListener = newWindowLifecycleListener(context);
        WindowManager<Record<X>> manager = new WindowManager<>(lifecycleListener, new ConcurrentLinkedQueue<>());
        ...
        EvictionPolicy<Record<X>, ?> evictionPolicy = getEvictionPolicy(windowConfig);
        TriggerPolicy<Record<X>, ?> triggerPolicy = getTriggerPolicy(windowConfig, manager,
    ...
    private TimestampExtractor<X> getTimeStampExtractor(WindowConfig windowConfig) {
        ...
        return (TimestampExtractor<X>) result;
    }

    private TriggerPolicy<Record<X>, ?> getTriggerPolicy(WindowConfig windowConfig, WindowManager<Record<X>> manager,
                                                         EvictionPolicy<Record<X>, ?> evictionPolicy, Context context) {
    ...
    private EvictionPolicy<Record<X>, ?> getEvictionPolicy(WindowConfig windowConfig) {
    ...
    protected WindowLifecycleListener<Event<Record<X>>> newWindowLifecycleListener(Context context) {
        return new WindowLifecycleListener<Event<Record<X>>>() {
            @Override
            public void onExpiry(List<Event<Record<X>>> events) {
                for (Event<Record<X>> event : events) {
                    event.getRecord().ack();
                }
            }

            @Override
            public void onActivation(List<Event<Record<X>>> tuples, List<Event<Record<X>>> newTuples,
                                     List<Event<Record<X>>>
    ...
    private void processWindow(Context context, List<Record<X>> tuples, List<Record<X>> newTuples, List<Record<X>>
            expiredTuples, Long referenceTime) {
    ...
    @Override
    public T process(X input, Context context) throws Exception {
        if (!this.initialized) {
            initialize(context);
        }

        Record<X> record = (Record<X>) context.getCurrentRecord();
    ...
    public T process(Window<Record<X>> inputWindow, WindowContext context) throws Exception {
        if (this.bareWindowFunction != null) {
            Collection<X> newCollection = inputWindow.get().stream().map(Record::getValue).collect(Collectors.toList());
